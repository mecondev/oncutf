...
import time
from typing import List, Any, Callable, Generator, Dict
...
class BatchProcessor:
    ...
    def get_stats(self) -> Dict[str, any]:
        ...
        return {
            ...
            "items_per_second": self.stats["total_items"] / self.stats["total_time"] if self.stats["total_time"] > 0 else 0
        }
...
class SmartBatchProcessor(BatchProcessor):
    ...
    def process_batches_optimized(self, items: List[Any], processor_func: Callable) -> List[Any]:
        ...
        if sample_time > self.optimization_threshold:
            ...
            logger.debug(f"[SmartBatchProcessor] Reducing batch size from {current_batch_size} to {new_batch_size}")
            ...
            logger.debug(f"[SmartBatchProcessor] Increasing batch size from {current_batch_size} to {new_batch_size}")
            self.batch_size = new_batch_size
