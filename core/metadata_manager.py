...
class MetadataManager:
    ...
    def load_metadata_for_items(
        ...
        """
        logger.warning(
            "[MetadataManager] This method is deprecated - use UnifiedMetadataManager instead"
        )
        ...
        logger.info(
            f"[MetadataManager] Delegated metadata loading for {len(items)} files to UnifiedMetadataManager"
        )
            ...
            if metadata:
                ...
                if self.parent_window and hasattr(self.parent_window, "metadata_cache"):
                    self.parent_window.metadata_cache.set(
                        file_item.full_path, metadata, is_extended=False
                    )
                ...
                if self.parent_window and hasattr(self.parent_window, "file_model"):
                    try:
                        ...
                        logger.warning(
                            f"[Loader] Failed to emit dataChanged for {file_item.filename}: {e}"
                        )
    ...
    def _load_files_with_worker(
        ...
        if hasattr(self, "metadata_worker") and self.metadata_worker:
            logger.debug(
                "[MetadataManager] Cleaning up existing metadata worker before starting new one...",
                extra={"dev_only": True},
            )
        ...
        loading_dialog.set_status(
            "Preparing metadata loading..."
            if not use_extended
            else "Preparing extended metadata loading..."
        )
        ...
        self.metadata_worker.size_progress.connect(
            lambda processed, total: self._on_metadata_size_progress(
                loading_dialog, processed, total
            )
        )
        ...
        try:
            ...
            for module in self.parent_window.rename_modules_area.findChildren(QWidget):
                if hasattr(module, "refresh_metadata_keys"):
                    logger.debug(
                        f"[MetadataManager] Refreshing metadata widget: {module.__class__.__name__}"
                    )
    ...
    def _on_file_metadata_loaded(self, file_path: str) -> None:
        ...
        try:
            ...
            if self._metadata_files_loaded % 10 == 0:
                logger.debug(
                    f"[MetadataManager] Refreshing metadata widgets after {self._metadata_files_loaded} files loaded"
                )
    ...
    def _cleanup_metadata_worker_and_thread(self) -> None:
        ...
        try:
            ...
            if hasattr(self, "metadata_worker") and self.metadata_worker:
                logger.debug(
                    "[MetadataManager] Cleaning up metadata worker...", extra={"dev_only": True}
                )
                try:
            ...
            if hasattr(self, "metadata_thread") and self.metadata_thread:
                logger.debug(
                    "[MetadataManager] Cleaning up metadata thread...", extra={"dev_only": True}
                )
                try:
                    ...
                    if self.metadata_thread.wait(2000):  # 2 second timeout
                        logger.debug(
                            "[MetadataManager] Thread finished gracefully", extra={"dev_only": True}
                        )
                    else:
                        logger.debug(
                            "[MetadataManager] Thread cleanup taking longer than expected, allowing background termination",
                            extra={"dev_only": True},
                        )
                    ...
                    thread_error = None
                    logger.debug(
                        f"[MetadataManager] Thread cleanup completed: {thread_error}",
                        extra={"dev_only": True},
                    )

                except Exception as thread_error:
                    logger.warning(
                        f"[MetadataManager] Error cleaning up metadata thread: {thread_error}"
                    )
                finally:
            ...
            logger.debug(
                f"[MetadataManager] Worker/thread cleanup completed: {e}", extra={"dev_only": True}
            )
        ...
        try:
            ...
            for file_item in selected_files:
                if metadata_tree_view.has_modifications_for_file(file_item.full_path):
                    ...
                    logger.debug(
                        f"[MetadataManager] Selected file with modifications: {file_item.filename}",
                        extra={"dev_only": True},
                    )

            if not files_to_save:
                logger.info(
                    "[MetadataManager] No selected files have metadata modifications to save"
                )
                return
    ...
    def save_all_modified_metadata(self) -> None:
        ...
        try:
            ...
            if (
                hasattr(metadata_tree_view, "_current_file_path")
                and metadata_tree_view._current_file_path
            ):
                logger.debug(
                    f"[MetadataManager] Saving current file modifications before collecting all: {metadata_tree_view._current_file_path}",
                    extra={"dev_only": True},
                )
            ...
            logger.debug(
                f"[MetadataManager] Got modifications for {len(all_modified_metadata)} files:",
                extra={"dev_only": True},
            )
            for file_path, modifications in all_modified_metadata.items():
                logger.debug(
                    f"[MetadataManager]   - {file_path}: {list(modifications.keys())}",
                    extra={"dev_only": True},
                )
                ...
                for file_item in all_files:
                    logger.debug(
                        f"[MetadataManager] Looking for FileItem with path: {file_path}",
                        extra={"dev_only": True},
                    )
                    if paths_equal(file_item.full_path, file_path):
                        files_to_save.append(file_item)
                        logger.debug(
                            f"[MetadataManager] MATCH found for: {file_item.filename}",
                            extra={"dev_only": True},
                        )
                        break

            logger.debug(
                f"[MetadataManager] Found {len(files_to_save)} FileItems to save",
                extra={"dev_only": True},
            )
            ...
            if metadata_tree_view and hasattr(metadata_tree_view, "_current_file_path"):
                if paths_equal(metadata_tree_view._current_file_path, file_path):
                    ...
                    if current_modifications:
                        logger.debug(
                            f"[MetadataManager] Metadata to save: {list(current_modifications.keys())}",
                            extra={"dev_only": True},
                        )
                        logger.debug(
                            f"[MetadataManager] Metadata values: {current_modifications}",
                            extra={"dev_only": True},
                        )
        ...
        try:
            ...
            for file_item in files_to_save:
                try:
                    ...
                    modified_metadata = self._get_modified_metadata_for_file(
                        file_item.full_path, all_modified_metadata
                    )

                    if modified_metadata:
                        logger.debug(
                            f"[MetadataManager] Metadata to save: {list(modified_metadata.keys())}",
                            extra={"dev_only": True},
                        )
                        logger.debug(
                            f"[MetadataManager] Metadata values: {modified_metadata}",
                            extra={"dev_only": True},
                        )
                        ...
                        for key_path, new_value in modified_metadata.items():
                            try:
                                metadata_manager.set_metadata_value(
                                    file_item.full_path, key_path, new_value
                                )
                            except Exception as field_error:
                                logger.error(
                                    f"[MetadataManager] Failed to save {key_path} for {file_item.filename}: {field_error}"
                                )
                                raise field_error
                        ...
                        success_count += 1
                        logger.info(
                            f"[MetadataManager] Successfully saved metadata for: {file_item.filename}"
                        )

                    else:
                        logger.warning(
                            f"[MetadataManager] No modifications found for: {file_item.filename}"
                        )

                except Exception as file_error:
                    logger.error(
                        f"[MetadataManager] Failed to save metadata for {file_item.filename}: {file_error}"
                    )
    ...
    def _update_file_after_save(self, file_item, saved_metadata: dict = None):
        ...
        try:
            ...
            if saved_metadata:
                logger.debug(
                    f"[MetadataManager] Updating cache with saved metadata for: {file_item.filename}",
                    extra={"dev_only": True},
                )
                if hasattr(file_item, "metadata") and file_item.metadata:
                    for key_path, new_value in saved_metadata.items():
                        logger.debug(
                            f"[MetadataManager] Updating cache: {key_path} = {new_value}",
                            extra={"dev_only": True},
                        )
            ...
            if file_table_model:
                ...
                for row in range(file_table_model.rowCount()):
                    ...
                    if model_file_item and paths_equal(
                        model_file_item.full_path, file_item.full_path
                    ):
                        # Update the model
            ...
            if metadata_tree_view and hasattr(metadata_tree_view, "_current_file_path"):
                if paths_equal(metadata_tree_view._current_file_path, file_item.full_path):
                    logger.debug(
                        f"[MetadataManager] Refreshing metadata view for updated file: {file_item.filename}",
                        extra={"dev_only": True},
                    )
    ...
    def _show_save_results(self, success_count, failed_files, files_to_save):
        ...
        if success_count > 0:
            if failed_files:
                ...
                message = (
                    f"Successfully saved {success_count} file(s), but failed to save:\n"
                    + "\n".join(failed_files)
                )
                ...
                if hasattr(self.parent_window, "set_status"):
                    self.parent_window.set_status(
                        status_msg, color=STATUS_COLORS["metadata_success"], auto_reset=True
                    )
        elif failed_files:
    ...
    def _get_files_from_source(self, source: str) -> list[FileItem] | None:
        ...
        if source == "selected":
            ...
            if hasattr(self.parent_window, "file_table_view") and hasattr(
                ...
                return [
                    self.parent_window.file_model.files[r]
    ...
    def cleanup(self) -> None:
        ...
        try:
            ...
            if hasattr(self, "metadata_worker") and self.metadata_worker:
                ...
                if hasattr(self.metadata_worker, "reader") and self.metadata_worker.reader:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"[MetadataManager] Error closing MetadataWorker ExifTool: {e}"
                        )
            ...
            try:
                logger.info(
                    "[MetadataManager] Force cleaning up any remaining ExifTool processes..."
                )
                from utils.exiftool_wrapper import ExifToolWrapper
