...
from core.pyqt_imports import pyqtSignal, QObject
...
class UnifiedPreviewManager:
    ...
    def generate_preview(
        ...
        name_pairs = self._generate_name_pairs(
            ...
            hash_availability, metadata_availability
        )
        ...
        if elapsed > 0.05:  # Log slow preview generation
            logger.info(f"[UnifiedPreviewManager] Preview generation took {elapsed:.3f}s for {len(files)} files")

        return result

    def _generate_cache_key(self, files: List[FileItem], modules_data: List[Dict], post_transform: Dict) -> str:
        ...
        for idx, file in enumerate(files):
            try:
                ...
                new_fullname = self._apply_modules_with_context(
                    file, modules_data, idx, metadata_cache,
                    hash_availability, metadata_availability
                )
...
class UnifiedExecutionManager:
    ...
    def _build_execution_plan(self, files: List[FileItem], new_names: List[str]) -> List[ExecutionItem]:
...
class UnifiedRenameEngine(QObject):
    ...
    def generate_preview(
        ...
        result = self.preview_manager.generate_preview(files, modules_data, post_transform, metadata_cache)
    ...
    def execute_rename(
        ...
        result = self.execution_manager.execute_rename(files, new_names, conflict_callback, validator)
