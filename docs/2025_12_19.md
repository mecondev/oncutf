# OnCutF — Master Plan & Status

**Date:** 2025-12-19 (Updated: 2025-12-20)  
**Status:** Phases 0-6 ✅ COMPLETE | Phase 7 (Polish) IN PROGRESS  
...
|-------|------|--------|----------|------------|
| 0 | Package Migration | ✅ | Dec 15 | Moved all code to `oncutf/` package |
| 1 | Controllers Architecture | ✅ | Dec 16 | 4 new controllers, MVC separation |
| 2 | State Management | ✅ | Dec 16-17 | FileStore, SelectionStore |
| 3 | Metadata Module Fix | ✅ | Dec 17-18 | UI/logic separation, 54% less code |
| 4 | Text Removal & Directory Org | ✅ | Dec 18-20 | 10 subdirectories in core/, 31 modules organized |
| 5 | Theme Consolidation | ✅ | Dec 18 | ThemeManager unified, -66% theme systems |
| 6 | Domain Layer Purification | ✅ | Dec 18-19 | Services layer, 5 protocols, DI support |
| 7 | Final Polish | 🔄 | Dec 19-20 | Documentation, cleanup |

**Current State:**
- **Architecture:** Clean MVC-inspired layers (UI → Controllers → Services → Domain)
...
1. **[x] Run mypy and fix any remaining errors** — ✅ Clean (0 errors)
2. **[x] Update README.md** — ✅ Fixed broken links, updated structure
3. **[x] Update ARCHITECTURE.md** — ✅ Fixed broken links, updated references
4. **[x] Validate internal doc links** — ✅ All links now point to active docs
5. **[ ] Final integration test** — Manual verification of all features
6. **[ ] Tag release candidate** — `v2.0.0-rc1` for the refactored codebase
...
├── services/             # Service protocols (Phase 6)
│   ├── interfaces.py     # 5 protocol definitions
│   ├── exiftool_service.py
│   ├── hash_service.py
│   └── filesystem_service.py
├── ui/                   # PyQt5 widgets & views
└── utils/                # Utilities
```
...
### Code Organization
- **God classes eliminated:** `unified_metadata_manager.py` (91KB) → 4 focused modules
- **Event handling split:** `event_handler_manager.py` (1528 lines) → 171-line facade + 3 modules
- **Core directory organized:** 50+ flat files → 10 logical subdirectories

### Performance
- **Startup:** 1697ms (264ms imports, 1390ms Qt init) — baseline documented
...
| File | Purpose |
|------|---------|
| `2025_12_19.md` | **This file** — master plan and status |
...
- `_archive/refactor-runs/` — All PHASE*_*.md files (24 documents)
...
|------|------------|--------|
| Breaking existing functionality | Run tests after each change | ✅ Active |
| Import cycle issues | Use lazy imports, TYPE_CHECKING | ✅ Implemented |
| Performance regression | Profile before/after | ✅ Baselined |
| Qt object lifecycle crashes | deleteLater(), defensive checks | ✅ Implemented |
