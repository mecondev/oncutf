...
from oncutf.utils.events import Signal
from oncutf.utils.logging.logger_factory import get_cached_logger
from oncutf.utils.threading.worker_base import WorkerBase

logger = get_cached_logger(__name__)


class BaseHashWorker(WorkerBase):
    ...
    # Progress signals (no type parameters at runtime)
    progress_updated = Signal()  # current_file, total_files, current_filename
    size_progress = Signal()  # bytes_processed, total_bytes
    status_updated = Signal()  # status message

    # Result signals
    duplicates_found = Signal()  # {hash: [file_paths]}
    comparison_result = Signal()  # comparison results
    checksums_calculated = Signal()  # {file_path: hash}

    # Control signals
    finished_processing = Signal()  # success flag
    error_occurred = Signal()  # error message
    file_hash_calculated = Signal()  # file_path, hash_value
