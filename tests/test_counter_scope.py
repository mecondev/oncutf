...
class TestCounterModuleEmitSignals:
    ...
    def test_scope_change_emits_signal(self, qtbot):
        """Test that changing scope emits the updated signal."""
        counter = CounterModule()
        qtbot.addWidget(counter)
        
        signal_emitted = False
        
        def on_updated(module):
            nonlocal signal_emitted
            signal_emitted = True
        
        counter.updated.connect(on_updated)
        
        # Use waitSignal context first, then change scope inside it
        with qtbot.waitSignal(counter.updated, timeout=1000):
            counter.scope_combo.setCurrentIndex(0)  # GLOBAL
