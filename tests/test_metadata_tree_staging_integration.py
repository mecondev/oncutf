...
class TestMetadataTreeStagingIntegration:
    ...
    def test_staging_integration(self, tree_view, staging_manager):
        ...
        with patch("core.metadata_staging_manager.get_metadata_staging_manager", return_value=staging_manager):
            ...
            # 1. Test display without staging
            display_data = {"EXIF": {"Artist": "Original Artist"}}
            tree_view._apply_modified_values_to_display_data(display_data)
            assert display_data["EXIF"]["Artist"] == "Original Artist"

            # 2. Stage a change
            staging_manager.stage_change(file_item.full_path, "EXIF/Artist", "New Artist")

            # 3. Test display with staging
            tree_view._apply_modified_values_to_display_data(display_data)
            assert display_data["EXIF"]["Artist"] == "New Artist"
