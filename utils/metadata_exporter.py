...
class MetadataExporter:
    ...
    def _export_files(self, files: List[Any], output_dir: str, format_type: str, scope: str) -> bool:
        ...
        try:
            ...
            if format_type == "csv":
                return self._export_csv(files, output_dir, scope)
    ...
    def _export_json(self, files: List[Any], output_dir: str, scope: str) -> bool:
        ...
        filename = f"metadata_export_{scope}_{timestamp}.json"
        ...
        try:
            ...
            logger.info(f"[MetadataExporter] JSON export completed: {output_path}")
            return True
    ...
    def _export_markdown(self, files: List[Any], output_dir: str, scope: str) -> bool:
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"metadata_export_{scope}_{timestamp}.md"
        output_path = os.path.join(output_dir, filename)
        ...
        try:
            with open(output_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"![OnCutF Logo](../assets/oncut_logo_dark_w_white_BG.png)\n\n")
                f.write(f"**Generated by:** {self.app_name} {self.app_version}  \n")
                f.write(f"**Export Date:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}  \n")
                f.write(f"**Scope:** {scope.title()} files  \n")
                ...
                for i, file_item in enumerate(files, 1):
                    ...
                    f.write(f"**Full Path:** `{file_data['full_path']}`  \n")
            ...
            logger.info(f"[MetadataExporter] Markdown export completed: {output_path}")
            return True
    ...
    def _export_csv(self, files: List[Any], output_dir: str, scope: str) -> bool:
        """Export metadata in CSV format."""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"metadata_export_{scope}_{timestamp}.csv"
        output_path = os.path.join(output_dir, filename)
        ...
        try:
            with open(output_path, 'w', newline='', encoding='utf-8') as f:
                ...
                # Header
                writer.writerow([
                    'File', 'Full_Path', 'File_Size', 'Hash_Algorithm', 'Hash_Value',
                    'Metadata_Type', 'Group', 'Key', 'Value', 'Is_Extended'
                ])
                ...
                for file_item in files:
                    ...
                    hash_algo = file_data.get('hash_info', {}).get('algorithm', '')
                    hash_value = file_data.get('hash_info', {}).get('value', '')
                    metadata_type = file_data['metadata_type']
                    ...
                    if file_data.get('metadata_groups'):
                        for group_name, group_data in file_data['metadata_groups'].items():
                            ...
                            for key, value in group_data.get('items', {}).items():
                                is_extended = key in extended_keys_list
                                writer.writerow([
                                    filename, full_path, file_size, hash_algo, hash_value,
                                    metadata_type, group_name, key, str(value), is_extended
                                ])
                    else:
            ...
            logger.info(f"[MetadataExporter] CSV export completed: {output_path}")
            return True

        except Exception as e:
            logger.error(f"[MetadataExporter] CSV export failed: {e}")
            return False
