    ...
    for filename in qss_files:
        ...
        if os.path.exists(path):
            with open(path, "r", encoding="utf-8") as f:
                ...
                full_style += qss + "\n"
                logger.debug(f"[THEME] Loaded {filename} ({len(qss)} characters)", extra={"dev_only": True})
        else:
            logger.warning(f"[THEME] QSS  file not found: {filename}")
...
def _debug_svg_paths(qss_content: str, project_root: str) -> None:
    ...
    for url in urls:
        ...
        if os.path.exists(file_path):
            logger.debug(f"[THEME] SVG icon found: {file_path}", extra={"dev_only": True})
        else:
            logger.warning(f"[THEME] SVG icon missing: {file_path}")
