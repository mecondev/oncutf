...
class FileTableView(QTableView):
    ...
    def show_context_menu(self, position: QPoint) -> None:
        """Show context menu with metadata export options."""
        # Only show context menu if we have files
        ...
        selected_rows = self._get_current_selection()
        total_files = self.model().rowCount()

        # Export selected files (only if files are selected)
        if selected_rows:
            export_selected_action = QAction(f"Export Metadata for Selected Files ({len(selected_rows)})", self)
            export_selected_action.triggered.connect(lambda: self._export_metadata_dialog("selected"))
            menu.addAction(export_selected_action)

        # Export all files
        export_all_action = QAction(f"Export Metadata for All Files ({total_files})", self)
        export_all_action.triggered.connect(lambda: self._export_metadata_dialog("all"))
        menu.addAction(export_all_action)

        # Show menu at cursor position
        menu.exec_(self.mapToGlobal(position))

    def _export_metadata_dialog(self, scope: str) -> None:
        """Show export dialog and handle metadata export."""
        ...
        export_button.clicked.connect(lambda: self._handle_export(dialog, format_combo, scope))
    ...
    def _handle_export(self, dialog: 'QDialog', format_combo: 'QComboBox', scope: str) -> None:
        ...
        try:
            ...
            # Get parent window for exporter
            parent_window = self.parent()
            while parent_window and not hasattr(parent_window, 'file_model'):
                parent_window = parent_window.parent()

            exporter = MetadataExporter(parent_window)

            # Export based on scope
            if scope == "selected":
                success = exporter.export_selected_files(output_dir, format_type)
            else:
            ...
            logger.exception(f"[FileTableView] Export error: {e}")
