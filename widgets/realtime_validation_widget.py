...
class RealTimeValidationWidget(QWidget):
    ...
    def update_validation(self):
        ...
        try:
            ...
            valid_count = sum(1 for item in validation_result.items if item.is_valid and not item.is_duplicate and not item.is_unchanged)
    ...
    def _show_error_details(self, items: List):
        ...
        if error_messages:
            ...
            if len(error_messages) > 3:
                self.error_details_label.setText(self.error_details_label.text() + f"\n... and {len(error_messages) - 3} more")
